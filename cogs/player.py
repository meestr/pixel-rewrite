...
class Player(commands.Cog):
    ...
    async def help(self, ctx):
        embed = discord.Embed(color=discord.Color.blurple(), type='rich')
        embed.add_field(name='Utilities', value='``create``, ``bal``, ``help``', inline=True)
        embed.add_field(name='Games', value='``coin``, ``dice``, ``roulette``, ``crime``, ``beg``, ''``slots``',
                        inline=True)
        embed.add_field(name='Meme',
                        value='``threats``, ``clyde``, ``trump``, ``jpeg``, ``changemymind``, ``whowouldwin``, '
                              '``phcomment``, ``fact``, ``captcha``, ``iphonex``, ``deepfry``, ``magik``, ``trash``',
                        inline=True)
        embed.add_field(name="News",
                        value="``p!news headlines``, ``p!news search``", inline=True)
        embed.add_field(name='PokeDex', value="``p!pokedex pokemon``, ``p!pokemon berry``, ``p!pokemon type``", inline=True)
        embed.add_field(name='Animals', value='``p!dog``, ``p!cat``, ``p!shiba``, ``p!bird``', inline=True)
        embed.add_field(name='NSFW', value="``hentai``, ``4k``, ``gif``, ``gonewild``, ``ass``, ``rule34``", inline=True)
    ...
    async def inv(self, ctx):
        ...
        await ctx.message.author.send(embed=await macro.send(f"Hi!\nThanks for taking an interest in me. \n [The "
                                                             f"support server is here](https://discord.gg/zdUSNVM)\n "
                                                             f"[and the link to invite me is here]("
                                                             f"https://discordapp.com/api/oauth2/authorize?client_id"
                                                             f"=560695413406171156&permissions=0&scope=bot)"))
